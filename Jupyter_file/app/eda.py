    ...
    if df is not None:
        ...
        if not num_cols.empty:
            ...
            if selected_num_col:
                ...
                group_counts = df['class'].value_counts()
                fig, ax = plt.subplots()
        ...
        if "age" in df.columns:
            st.write("Age Stats:")
            st.write(f"Min: {df['age'].min()}, Max: {df['age'].max()}, Mean: {df['age'].mean():.2f}")
        ...
        if all(col in df.columns for col in ["pixelSpacing_X", "pixelSpacing_Y"]):
        ...
        if "viewPosition" in df.columns:
            ...
            ap = df[df["viewPosition"] == "AP"].shape[0]
            pa = df[df["viewPosition"] == "PA"].shape[0]
            st.write(f"AP: {ap}, PA: {pa}")
    else:
